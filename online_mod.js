...
(function () {
    ...
    function proxy(name) {
      ...
      if (Lampa.Storage.field('online_mod_proxy_' + name) === true) {
        ...
        if (name === 'animewost') return user_proxy1;
    ...
    function animewost(component, _object) {
      var network = new Lampa.Reguest();
      var extract = {};
      var object = _object;
      var select_title = '';
      ...
      function stripTags(str) {
        return component.decodeHtml((str || '').replace(/<[^>]*>/g, ' ')).replace(/\s+/g, ' ').trim();
      }

      function normalizeLabel(label, url) {
        var found = (label || '').match(/(\d{3,4})/);
        var quality = parseInt(found && found[1] || 0);
        if (!quality && /\/1080\//.test(url)) quality = 1080;
      ...
      function parseSearchItems(str) {
        var items = [];
        var matches = [];
        var re = /<h2>\s*<a href="([^"]+)">([\s\S]*?)<\/a>\s*<\/h2>/g;
        var match;
        ...
        matches.forEach(function (item, index) {
          var next = matches[index + 1] ? matches[index + 1].index : str.length;
          var block = str.substring(item.index, next);
          var release = item.link.match(/\/(\d+)-[^\/]+\.html/);
          var title = item.title.replace(/\s*\[[^\]]+\]\s*$/, '').trim();
          var parts = title.split(/\s+\/\s+/);
          var year = parseInt((block.match(/<p><strong>[^<]*<\/strong>\s*(\d{4})<\/p>/) || [])[1] || 0);
          var episodes_count = parseInt((item.title.match(/\[(?:\d+\s*-\s*)?(\d+)/) || [])[1] || 0);
          var alt = stripTags((block.match(/<h4[^>]*>([\s\S]*?)<\/h4>/) || [])[1] || '');
          items.push({
            id: release ? release[1] : item.link,
            title: parts[0] || title,
            orig_title: parts[1] || alt || title,
            full_title: title,
            year: year,
            episodes_count: episodes_count,
            link: component.fixLink(item.link, host)
          });
        });
      ...
      function parseEpisodes(str, card) {
        var episodes = [];
        var data = (str.match(/var\s+data\s*=\s*\{([\s\S]*?)\}\s*;/) || [])[1] || '';
        var re = /"([^"]+)"\s*:\s*"([^"]+)"/g;
        var match;

        while ((match = re.exec(data)) !== null) {
          var num = parseInt((match[1].match(/(\d+)/) || [])[1] || episodes.length + 1);
          episodes.push({
            id: match[2],
        ...
        if (!episodes.length && card.id) {
          episodes.push({
            id: card.id,
            title: card.title || select_title,
            orig_title: card.orig_title || card.title || select_title,
            quality: '480p ~ 1080p',
            info: ' / AnimeWost'
          });
      ...
      function parseFrameItems(str) {
        ...
        while ((match = re.exec(str)) !== null) {
          if (match[1].indexOf('http') !== -1 && match[1].length > file.length) file = match[1];
        }
      ...
      function append(items) {
        ...
        items.forEach(function (element) {
          ...
          var view = Lampa.Timeline.view(hash);
          var item = Lampa.Template.get('online_mod', element);
          var hash_file = Lampa.Utils.hash(element.season ? [element.season, element.season > 10 ? ':' : '', element.episode, object.movie.original_title, element.orig_title, 'animewost'].join('') : object.movie.original_title + element.orig_title + element.title);
          element.timeline = view;
          element.template = item;
          item.append(Lampa.Timeline.render(view));
    ...
    function component(object) {
      ...
      var all_sources = [{
        ...
        name: 'animewost',
        title: 'AnimeWost',
        source: new animewost(this, object),
        search: true,
        kp: false,
        imdb: false
      }, {
    ...
    function initStorage() {
      if (!Utils.isDebug()) {
        ...
        Lampa.Storage.set('online_mod_proxy_animewost', 'false');
      ...
      Lampa.Params.trigger('online_mod_proxy_animewost', false);
    ...
    function initSettings() {
      ...
      template += "\n        <div class=\"settings-param selector\" data-name=\"online_mod_proxy_animewost\" data-type=\"toggle\">\n            <div class=\"settings-param__name\">#{online_mod_proxy_balanser} AnimeWost</div>\n            <div class=\"settings-param__value\"></div>\n        </div>";
